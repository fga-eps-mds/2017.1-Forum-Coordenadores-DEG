        ...
        private async void DeleteForum() {
            ...
            if (answer == true) {
                if (await _forumService.DeleteForumAsync(RemoteId)) {
                    await _pageService.DisplayAlert("Fórum deletado", "O fórum foi excluído com sucesso", "OK");
                    await _pageService.PopAsync();
                }
                else {
                    await _pageService.DisplayAlert("Erro!", "O fórum não pôde ser deletado, tente novamente.", "OK");
                }
            }
        }
    }
